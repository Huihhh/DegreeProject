...
def get_dataset(self):
    logger.info(f'***** Preparing data: {self.name} *****')
    self.DATA = {
    ...
    if self.name in ['spiral', 'sphere']:
        ...
        self.trainset = get_torch_dataset(
            self.DATA[self.name](**{
                **self.CFG, 'Nfactor': Nfactor_train
            }))
        self.valset = get_torch_dataset(
            self.DATA[self.name](**{
                **self.CFG, 'Nfactor': Nfactor_val,
                'seed': self.seed + 1
            }))
        self.testset = get_torch_dataset(
            self.DATA[self.name](**{
                **self.CFG, 'Nfactor': Nfactor_test,
                'seed': 20
            }))
        ...
        self.trainset = get_torch_dataset(self.DATA[self.name](
            n_samples=n_train, seed=self.seed, shuffle=True))
        self.valset = get_torch_dataset(self.DATA[self.name](n_samples=n_val,
                                                             seed=self.seed +
                                                             1,
                                                             shuffle=False))
        self.testset = get_torch_dataset(self.DATA[self.name](n_samples=n_test,
                                                              seed=20,
                                                              **self.CFG))
...
def make_circles(width,
    ...
    if noise_ratio:
        X, y = add_noise([X, y], generator, noise_level)

    return X, y[:, None]


def make_moons(width,
               gap,
               n_samples=2000,
               noise_ratio=0,
               noise_level=0,
    ...
    if noise_ratio:
        X, y = add_noise([X, y], generator)

    return X, y[:, None]


def make_spiral(width,
                factor=50,
                noise_ratio=0,
                shuffle=True,
                seed=0,
    ...
    factor : int, default=50
            controls the number of points: 97 * 2 * factor
    width : float, default=None
        Standard deviation of Gaussian noise added to the data. width of the circles
    noise_ratio: float, betwen 0-1, defines the number of noisy points
    noise_level: float, betwen 0-1, defines the level a point is diverging from the sample
    ...
    x1 = np.array(spiral(1, factor))
    x2 = np.array(spiral(-1, factor))
...
def make_sphere(self, Nfactor=50, seed=0, *args, **kwargs):
    ...
    x1, y1 = make_sphere(self.CFG.r,
                         self.CFG.w,
                         sign=0,
                         center=self.CFG.center0,
                         n=n)
    x2, y2 = make_sphere(self.CFG.r,
                         self.CFG.w,
                         sign=1,
                         center=self.CFG.center1,
                         n=n)
...
def add_noise(self, data, noise_ratio, noise_level, generator):
    X, y = data
    ...
    def make_points_to_plot_LR(self, use_grid=True):
        if use_grid:
            ...
            if self.name in ['spiral', 'sphere']:
                return self.DATA[self.name](Nfactor=self.CFG.Nfactor * 2)
            else:
                return self.DATA[self.name](n_samples=self.CFG.total_samples *
                                            2)
    ...
    def plot(self, save_dir='./'):
        ...
        plt.ylabel('y')
        plt.title(
            f'{self.name} noise/total points:{self.CFG.noise_ratio} noise(std):{self.CFG.noise_level}'
        )
