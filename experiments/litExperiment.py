...
class LitExperiment(pl.LightningModule):
    ...
    def plot_signatures(self):
        ...
        total_regions['non_empty_regions'] = boundary_regions['non_empty_regions'] + red_regions[
            'non_empty_regions'] + blue_regions['non_empty_regions']
        ...
        if self.CFG.plot_LR:
            if (self.current_epoch == 0) or (self.current_epoch + 1) % (self.CFG.plot_every * 10) == 0:
                ...
                self.log(f'Linear Regions[Image]/epoch{self.current_epoch}', wandb.Image(fig))
                plt.close(fig)
